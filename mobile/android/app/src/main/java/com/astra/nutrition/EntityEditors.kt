...
private data class VariantDraft(var machine: String = "", var equipment: String = "", var description: String = "", var technique: String = "", var tips: String = "")
...
fun ExerciseEditorDialog(existing: Exercise?, onSave: (JSONObject) -> Unit, onDelete: (() -> Unit)?, onDismiss: () -> Unit) {
    ...
    val variants = remember { mutableStateListOf<VariantDraft>().also { list -> existing?.variants?.forEach { list.add(VariantDraft(it.machine.orEmpty(), it.equipment.orEmpty(), it.description.orEmpty(), it.technique.orEmpty(), it.tips.orEmpty())) } } }
...
fun WorkoutPlanEditorDialog(plan: WorkoutPlan?, complex: WorkoutComplex?, exercises: List<Exercise>, onSave: (JSONObject) -> Unit, onDismiss: () -> Unit) {
    var scheduledAt by remember { mutableStateOf(plan?.scheduledAt ?: todayTime()) }; var duration by remember { mutableStateOf(plan?.duration?.toString().orEmpty()) }
