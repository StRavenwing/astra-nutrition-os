...
private fun JSONObject.optTextOrNull(key: String): String? = optString(key).trim().takeIf { it.isNotBlank() && !it.equals("null", ignoreCase = true) }
...
private fun JSONObject.planItem() = WorkoutPlanItem(if (has("id") && !isNull("id")) optInt("id") else null, optInt("exercise_id"), optTextOrNull("name"), optTextOrNull("muscle_group"), optDoubleOrNull("working_weight"), optDoubleOrNull("sets"), optDoubleOrNull("duration_minutes"), optDoubleOrNull("speed_kmh"), optInt("variant_id").takeIf { it > 0 })
private fun JSONObject.plan() = WorkoutPlan(optInt("id"), optString("scheduled_at"), optDoubleOrNull("duration_minutes"), optString("status"), optTextOrNull("completed_at"), optJSONArray("items").planItems(), optTextOrNull("name"))
