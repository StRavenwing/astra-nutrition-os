...
private enum TrainerWorkspaceSection: String, CaseIterable, Identifiable, Hashable {
...
private struct ExerciseVariantDraft: Identifiable {
    let id = UUID()
    var name = ""
    var machine = ""
    var equipment = ""
    ...
    init(_ variant: ExerciseVariant? = nil) {
        name = variant?.name ?? ""
    ...
    @ViewBuilder private var variantFields: some View {
        Section("Варианты выполнения") {
            ForEach(variants.indices, id: \.self) { index in
                VStack(alignment: .leading, spacing: 6) {
                    Text("Вариант \(index + 1)").font(.caption.weight(.semibold)).foregroundStyle(AstraTheme.blue)
                    Text("Введите название").font(.caption).foregroundStyle(AstraTheme.muted)
                    ...
                    if variants.count > 1 {
                        Button("Удалить вариант", role: .destructive) { variants.remove(at: index) }
                    }
                }
            }
...
struct WorkoutPlanEditorMobileView: View {
    ...
    @State private var name = ""; @State private var scheduledAt = ""; @State private var duration = ""; @State private var exerciseId = 0; @State private var variantId: Int? = nil; @State private var weight = ""; @State private var sets = ""; @State private var itemDuration = ""; @State private var speed = ""
    init(initial: WorkoutPlan?, complex: WorkoutComplex?, exercises: [Exercise], onSave: @escaping (JSONPayload) -> Void) { self.initial = initial; self.complex = complex; self.exercises = exercises; self.onSave = onSave; let item = initial?.items.first ?? complex?.items.first.map { WorkoutPlanItem(id: $0.id, exerciseId: $0.exerciseId, name: $0.name, muscleGroup: $0.muscleGroup, workingWeight: $0.workingWeight, sets: $0.sets, durationMinutes: $0.durationMinutes, speedKmh: $0.speedKmh) }; _name = State(initialValue: initial?.name ?? ""); _scheduledAt = State(initialValue: initial?.scheduledAt ?? ""); _duration = State(initialValue: initial?.durationMinutes?.display ?? ""); _exerciseId = State(initialValue: item?.exerciseId ?? exercises.first?.id ?? 0); _variantId = State(initialValue: item?.variantId); _weight = State(initialValue: item?.workingWeight?.display ?? ""); _sets = State(initialValue: item?.sets?.display ?? ""); _itemDuration = State(initialValue: item?.durationMinutes?.display ?? ""); _speed = State(initialValue: item?.speedKmh?.display ?? "") }
    var body: some View { NavigationStack { Form { TextField("Название тренировки", text: $name); TextField("Дата и время", text: $scheduledAt); TextField("Длительность, мин", text: $duration).keyboardType(.decimalPad); if !exercises.isEmpty { Picker("Упражнение", selection: Binding(get: { exerciseId }, set: { exerciseId = $0; variantId = nil })) { ForEach(exercises) { Text($0.name).tag($0.id) } } }; if let selectedExercise = exercises.first(where: { $0.id == exerciseId }), !selectedExercise.variants.isEmpty { Picker("Вариант", selection: Binding(get: { variantId ?? 0 }, set: { variantId = $0 == 0 ? nil : $0 })) { Text("Без варианта").tag(0); ForEach(Array(selectedExercise.variants.enumerated()), id: \.offset) { index, variant in Text(variant.name?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false ? variant.name! : "Вариант \(index + 1)").tag(variant.id ?? 0) } } }; Section("Параметры упражнения") { TextField("Вес", text: $weight).keyboardType(.decimalPad); TextField("Подходы", text: $sets).keyboardType(.decimalPad); TextField("Длительность, мин", text: $itemDuration).keyboardType(.decimalPad); TextField("Скорость, км/ч", text: $speed).keyboardType(.decimalPad) } }.navigationTitle(initial == nil ? "Включить в план" : initial?.id == 0 ? "Повторить тренировку" : "Редактировать план").toolbar { ToolbarItem(placement: .cancellationAction) { Button("Отмена") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Сохранить") { let item = JSONPayload(values: ["exercise_id": AnyEncodable(exerciseId), "variant_id": AnyEncodable(variantId), "working_weight": editorOptionalNumber(weight), "sets": editorOptionalNumber(sets), "duration_minutes": editorOptionalNumber(itemDuration), "speed_kmh": editorOptionalNumber(speed)]); onSave(JSONPayload(values: ["name": editorOptionalString(name), "scheduled_at": AnyEncodable(scheduledAt), "duration_minutes": editorOptionalNumber(duration), "items": AnyEncodable([item])])); dismiss() } } } } }
}
