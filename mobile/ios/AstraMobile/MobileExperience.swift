...
struct TrainerPlanDetailView: View { @Environment(\.dismiss) private var dismiss; let plan: WorkoutPlan; var body: some View { NavigationStack { List { Section { Text(plan.name?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false ? plan.name! : "Тренировка").font(.title2.weight(.bold)); Text(plan.scheduledAt).font(.subheadline); Text(plan.status).foregroundStyle(AstraTheme.muted) }; Section("Упражнения") { ForEach(plan.items) { item in VStack(alignment: .leading) { Text(item.name ?? "Упражнение").font(.headline); Text("\(item.sets.display) подхода · \(item.durationMinutes.display) мин").font(.caption).foregroundStyle(AstraTheme.muted) } } } }.navigationTitle("План").toolbar { ToolbarItem(placement: .cancellationAction) { Button("Закрыть") { dismiss() } } } } } }
